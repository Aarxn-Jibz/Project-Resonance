...
export default function StemPlayer({ audioSource = null, midiData = null, onOpenSheet, onPlayStateChange, ref }) {
  ...
  return (
    <div className="w-full max-w-2xl bg-[#111] border border-gray-800 p-3 sm:p-6 rounded-lg font-mono relative z-40">

      {/* Global Controls */}
      <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4 border-b border-gray-800 pb-5 sm:pb-6 mb-5 sm:mb-6">
        <div className="flex items-center gap-3 sm:gap-4 min-w-0">
          <button
            onClick={togglePlay}
            disabled={!audioSource}
            className={`w-12 h-12 sm:w-14 sm:h-14 shrink-0 rounded-full flex items-center justify-center transition-colors text-white
              ${!audioSource ? 'bg-gray-900 cursor-not-allowed' : 'bg-gray-800 hover:bg-gray-700'}`}
          >
            {isPlaying ? <Pause className="w-6 h-6" /> : <Play className="w-6 h-6 ml-1" />}
          </button>
          <div className="min-w-0">
            <h3 className="text-gray-300 tracking-wider sm:tracking-widest text-xs sm:text-sm">LATENT_STEM_MIXER</h3>
        ...
        {audioSource && (
          <button
            ...
            className="min-h-11 w-full sm:w-auto font-mono text-xs text-gray-400 hover:text-white hover:bg-white/10 px-4 py-2 rounded transition-colors border border-gray-800"
          >
            PLAY ORIGINAL
          </button>
        )}
      ...
      <div className="space-y-4">
        {STEM_CONFIGS.map((stem) => (
          <div key={stem.id} className="flex flex-wrap sm:flex-nowrap items-center justify-between gap-y-3 bg-[#1a1a1a] p-3 rounded border border-gray-800/50">
            <audio
            ...
            <div className="flex items-center gap-3 w-auto sm:w-48 min-w-0">
              <AudioWaveform className={`w-5 h-5 ${stem.color}`} />
              <span className={`text-[11px] sm:text-xs tracking-wider sm:tracking-widest truncate ${stem.color}`}>{stem.name}</span>
            </div>

            <div className="order-3 sm:order-none basis-full sm:basis-auto flex items-center gap-3 flex-1 px-1 sm:px-8">
              <button onClick={() => toggleMute(stem.id)} className="min-w-11 min-h-11 flex items-center justify-center text-gray-500 hover:text-white transition-colors">
            ...
            <div className="flex items-center gap-1 sm:gap-2">
              {midiData?.[stem.id] && (
                <button
                  ...
                  className="min-w-11 min-h-11 flex items-center justify-center text-gray-500 hover:text-white hover:bg-white/10 rounded transition-all"
              ...
              <button
                ...
                className="min-w-11 min-h-11 flex items-center justify-center text-gray-500 hover:text-[#4ade80] hover:bg-green-400/10 rounded transition-all cursor-pointer"
                title="Download Stem"
              >
